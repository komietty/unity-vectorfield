...
namespace VectorField {
    ...
    public static class ExteriorDerivatives {
        ...
        public static Sparse BuildHodgeStar0Form(HeGeom g) {
            var n = g.nVerts;
            var r = new double[n];
            for (var i = 0; i < n; i++) {
                var v = g.Verts[i];
            ...
            return Sparse.OfDiagonalArray(r);
        }

        public static Sparse BuildHodgeStar1Form(HeGeom g) {
            var n = g.nEdges;
            var r = new double[n];
            for (var i = 0; i < n; i++) {
                var e = g.Edges[i];
        ...
        public static Sparse BuildHodgeStar2Form(HeGeom g) {
            var n = g.nFaces;
            var r = new double[n];
            for (var i = 0; i < n; i++) {
                var f = g.Faces[i];
        ...
        public static Sparse BuildExteriorDerivative0Form(HeGeom g) {
            var l = new List<(int, int, double)>();
            foreach (var e in g.Edges) {
                l.Add((e.eid, g.halfedges[e.hid].vid, -1));
                l.Add((e.eid, g.halfedges[e.hid].next.vid,  1));
            }
            return Sparse.OfIndexed(g.nEdges, g.nVerts, l);
        }

        public static Sparse BuildExteriorDerivative1Form(HeGeom g) {
            var l = new List<(int, int, double)>();
            ...
            return Sparse.OfIndexed(g.nFaces, g.nEdges, l);
        }
        ...
        */
        public static float3[] InterpolateWhitney(Vector oneForm, HeGeom g) {
