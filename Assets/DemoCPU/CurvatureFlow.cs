...
namespace ddg {
    public class CurvatureFlow : CurvatureView {
        ...
        IEnumerator Smooth() {
            flow = new MeanCurvatureFlow(geom, native);
    ...
    public class MeanCurvatureFlow {
        [DllImport("EigenDll")] static extern void SolveLU(int ntrps, int nvrts, [In] Trp[] trps, [In] Vector3[] vrts, [Out] Vector3[] outs);
        HalfEdgeGeom geom;
        SparseMatrix L;
        bool native = false;
        int nv => geom.nVerts;
        
        public MeanCurvatureFlow(HalfEdgeGeom geom, bool native) {
            this.geom = geom;
            this.native = native;
            L = GenLaplaceMtx();
        }
